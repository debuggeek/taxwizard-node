...
exports.resetBatchProp = async function resetBatchProp(propId){
    ...
    let dbRes = null;
    let baseQuery = "UPDATE `"+cfg.batchPropTable+"` " +
                    "SET `completed` = 'false', " +
                    "`TotalComps` = NULL, " +
                    "`ErrorSeen` = NULL ";
...
exports.purgeAllBatch = async function purgeAllBatch(){
    let query = "TRUNCATE `"+cfg.batchPropTable+"`";
...
exports.getBatchProps_2018 = async function getBatchProps_2018(limit, start){
    dbRes = await db.conn.queryPromise("SELECT prop,prop_mktval,"+
        ...
        "Median_Eq11,TotalComps FROM `"+cfg.batchPropTable+"` WHERE completed = 'true' LIMIT ? OFFSET ?",
        [limit, start]);
...
exports.getBatchProps_2019 = async function getBatchProps_2019(limit, start){
    dbRes = await db.conn.queryPromise("SELECT prop,prop_mktval,"+
        ...
        "Comp10_IndicatedValue,TotalComps,CompType " +
        "FROM `"+cfg.batchPropTable+"` WHERE completed = 'true' LIMIT ? OFFSET ?",
        [limit, start]);
