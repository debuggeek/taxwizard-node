...
exports.getCompsFor = async function (context, subj) {
  ...
  let whereQuery = {NeighborhoodCode: subjHood};
  if(context.percAbove && context.percBelow) {
    ...
    console.log("getHydratedSalesComps >> Sqft info not provided, finding comps of all size");
  }
  ...
  try {
    possibleComps = await SalesHistory.findAll({
      ...
      include: [{
        model: Property,
        where: sqftWhere,
        // attributes: ["QuickRefId", "TotalSqFtLivingArea", "SitusAddress", "NeighborhoodCode", "Class", "actualYear"],
        include: [
          {
            model: Improvement,
            as: "improvements",
